...
<body>
	...
	<div class="container">
		...
		<p>On our Apache server we have the [Google Pagespeed Module](http://code.google.com/p/modpagespeed/) installed so there are certain things that we don't do which we used to. Things like *combining multiple stylesheets into one* we don't do any more because the module handles that, and caches the combined files.</p>
		<h2>CSS</h2>		
		<p>The CSS is 'object-orientated' (OOCSS) as I feel that is the most scalable way to handle projects of any size.</p>
		<p>Although I disagree with the seemingly common tradition of using a CSS reset (mainly because nearly all of them take a 'scored earth policy' approach) I decided to use one. I chose to use the [normalise.css](https://github.com/necolas/normalize.css) reset. This was because it does a lot of things right (e.g. it leaves a lot of the default browser settings in place because they realise that you end up writing more CSS to work-around the reset than you would have written to work-around the browser defaults!) but also I only had to add two of my own customisations to it.</p>
		<p>I also liked the stylings for some of the elements (such as buttons, alert spans etc) from the [Twitter Bootstrap](https://github.com/twitter/bootstrap) framework, but I didn't want the entire framework - just those small items. So I took the framework and stripped out 95% of it.</p>
